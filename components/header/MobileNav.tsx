  ...
  return (
    ...
    onClick={()=>setMobileMenuOpen(false)} />
        {navItems.map((item) => (
            ...
            px-6 mb-4 hover:bg-white hover:text-[#0f3d2e] font-bold rounded py-1 transition">
            {item.name}
            {item.subItems && (
                <div 
                className="absolute bg-black/70 text-white backdrop-blur-sm
                top-12 rounded-lg shadow-lg transform py-2 w-48 
                opacity-0 invisible transition translate-y-4
                group-hover:opacity-100 group-hover:visible group-hover:translate-y-0"
                >
                    {item.subItems.map((subItem) => (
                        <a key={subItem.name} href={subItem.href} className="px-4 py-1 hover:bg-[#0f3d2e] hover:text-white cursor-pointer">
                            {subItem.name}
                        </a>
                    ))}
                </div>)}
